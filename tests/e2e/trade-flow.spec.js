...
test.describe('Trade Lifecycle', () => {
  ...
  test('can open a new LONG trade', async ({ page }) => {
    ...
    const addBtn = page.locator(
      'button:has-text("Add Trade"), button:has-text("New Trade"), button:has-text("Log Trade"), button[aria-label*="add" i]'
    )
    ...
    const longOption = page.locator(
      'button:has-text("LONG"), input[value="LONG"], option[value="LONG"]'
    )
    if ((await longOption.count()) > 0) await longOption.first().click()

    // Submit
    await page.click(
      'button[type="submit"]:has-text("Save"), button:has-text("Add"), button:has-text("Submit"), dialog button[type="submit"]'
    )
  ...
  test('can open a SHORT trade', async ({ page }) => {
    const addBtn = page.locator(
      'button:has-text("Add Trade"), button:has-text("New Trade"), button:has-text("Log Trade")'
    )
    await addBtn.first().click()
    ...
    const shortOption = page.locator(
      'button:has-text("SHORT"), input[value="SHORT"], option[value="SHORT"]'
    )
  ...
  test('can partial-close an OPEN trade', async ({ page }) => {
    ...
    if ((await partialBtn.count()) > 0) {
      ...
      await page.click(
        'button[type="submit"]:has-text("Close"), button:has-text("Confirm"), dialog button[type="submit"]'
      )
  ...
  test('can delete a trade', async ({ page }) => {
    ...
    if ((await deleteBtn.count()) > 0) {
      ...
      const confirmBtn = page.locator(
        'button:has-text("Confirm"), button:has-text("Yes, delete"), button:has-text("Delete")'
      )
...
test.describe('Trade Table Filtering', () => {
  ...
  test('status filter shows only OPEN trades', async ({ page }) => {
    const filterSelect = page.locator(
      'select[name*="status"], button:has-text("Filter"), [aria-label*="filter" i]'
    )
    if ((await filterSelect.count()) > 0) {
      ...
      const rows = page
        .locator('tr, [data-testid="trade-row"]')
        .filter({ hasText: /CLOSED|PARTIAL/ })
  ...
  test('search by symbol filters trades', async ({ page }) => {
    const searchInput = page.locator(
      'input[placeholder*="search" i], input[placeholder*="symbol" i], input[type="search"]'
    )
...
test.describe('Journal Entry', () => {
  ...
  test('can add a journal entry to a trade', async ({ page }) => {
    ...
    const journalTab = page.locator(
      'button:has-text("Journal"), tab:has-text("Journal"), a:has-text("Journal")'
    )
    if ((await journalTab.count()) > 0) {
      await journalTab.first().click()
      await page.waitForLoadState('networkidle')

      const addJournalBtn = page.locator(
        'button:has-text("Add"), button:has-text("New Entry"), button:has-text("Write")'
      )
      if ((await addJournalBtn.count()) > 0) {
        ...
        await expect(page.locator('text=Test journal entry from E2E test')).toBeVisible({
          timeout: 8000,
        })
      }
    } else {
