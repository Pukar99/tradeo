...
function calcAvgRR(trades) {
  const valid = trades.filter(
    (t) => t.status === 'CLOSED' && t.entry_price && t.sl && t.exit_price !== undefined
  )
...
describe('calcWinRate', () => {
  ...
  test('all losses = 0%', () =>
    expect(calcWinRate([{ status: 'CLOSED', realized_pnl: -100 }])).toBe(0))
  test('no closed trades = 0%', () =>
    expect(calcWinRate([{ status: 'OPEN', realized_pnl: 0 }])).toBe(0))
  test('empty array = 0%', () => expect(calcWinRate([])).toBe(0))
  test('breakeven (pnl=0) counts as loss', () =>
    expect(calcWinRate([{ status: 'CLOSED', realized_pnl: 0 }])).toBe(0))
})
...
describe('weightedAvgPrice', () => {
  test('equal quantities', () => expect(weightedAvgPrice(100, 500, 100, 600)).toBe(550))
  test('larger existing position dominates', () =>
    expect(weightedAvgPrice(900, 100, 100, 200)).toBe(110))
  test('same price = unchanged', () => expect(weightedAvgPrice(50, 400, 10, 400)).toBe(400))
...
describe('calcRemainingQty', () => {
  test('no partial exits = full quantity', () => expect(calcRemainingQty(200, [])).toBe(200))
  test('one partial exit', () => expect(calcRemainingQty(200, [{ exit_quantity: 50 }])).toBe(150))
  ...
  test('fully exited = 0', () => expect(calcRemainingQty(100, [{ exit_quantity: 100 }])).toBe(0))
})
