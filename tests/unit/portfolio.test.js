...
describe('calcPortfolioAllocation', () => {
  ...
  test('single holding = 100%', () => {
    const result = calcPortfolioAllocation([
      { symbol: 'NABIL', market_value: 50000, unrealized_pnl: 0 },
    ])
...
describe('calcTotalPnl', () => {
  test('sums unrealized P&L across holdings', () => {
    const holdings = [{ unrealized_pnl: 5000 }, { unrealized_pnl: -2000 }, { unrealized_pnl: 1000 }]
    expect(calcTotalPnl(holdings)).toBe(4000)
  })
  test('all losses', () =>
    expect(calcTotalPnl([{ unrealized_pnl: -100 }, { unrealized_pnl: -200 }])).toBe(-300))
