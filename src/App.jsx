    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gray-50 dark:bg-gray-950 text-gray-700 dark:text-gray-300 p-8 animate-fade-up">
          ...
          <div className="text-[11px] text-gray-400 max-w-sm text-center">
    ...
    () => ({
      ...
      '/': () => {
        if (location.pathname === '/screen') {
          document.querySelector('[data-chart-search]')?.focus()
        }
      },
    }),
  ...
  useEffect(() => {
    ...
    runMeroshareAutoApplyOnLogin()
      .then((res) => {
        ...
        applied.forEach((r) =>
          toast.success(`IPO Auto-Apply: ${r.label} — ${r.message}`, { duration: 6000 })
        )
        errors.forEach((r) =>
          toast.error(`IPO Auto-Apply failed: ${r.label} — ${r.message}`, { duration: 6000 })
        )
      })
      .catch(() => {})
  ...
  return (
    <NavbarAutoHideCtx.Provider value={ctxValue}>
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
        {!isAuthPage && (
          <Navbar autoHide={autoHideActive} hidden={navHidden} onMouseEnter={showNavbar} />
        )}
