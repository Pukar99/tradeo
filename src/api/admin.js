...
export const patchSystemConfig       = (key, val)  => API.patch(`/api/admin/system/config/${encodeURIComponent(key)}`, { value: val })

// ── Feature Flags ─────────────────────────────────────────────────────────────
export const getAllAdminFlags = ()              => API.get('/api/admin/flags')
export const patchAdminFlag  = (name, data)    => API.patch(`/api/admin/flags/${encodeURIComponent(name)}`, data)
...
export const patchAdminAnnouncement = (id, data)    => API.patch(`/api/admin/announcements/${id}`, data)
