...
const MOOD_META = {
  Bullish: {
    icon: '↑',
    pill: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800/40',
  },
  Bearish: {
    icon: '↓',
    pill: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border-red-200 dark:border-red-800/40',
  },
  Neutral: {
    icon: '→',
    pill: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300 border-gray-200 dark:border-gray-700',
  },
  Confused: {
    icon: '?',
    pill: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border-amber-200 dark:border-amber-800/40',
  },
  Excited: {
    icon: '⚡',
    pill: 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400 border-violet-200 dark:border-violet-800/40',
  },
}
...
function GalleryCard({ entry, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-center justify-between px-3 py-1.5 border-b border-gray-100 dark:border-gray-800/60 ${isUp ? 'bg-emerald-50/60 dark:bg-emerald-900/10' : 'bg-red-50/60 dark:bg-red-900/10'}`}
      >
        ...
        <div className="flex items-baseline gap-1.5">
          ...
          {change != null && (
            <span
              className={`text-[10px] font-bold tabular-nums ${isUp ? 'text-emerald-500' : 'text-red-400'}`}
            >
              {change >= 0 ? '+' : ''}
              {change.toFixed(2)}%
            </span>
      ...
      <div
        className="relative w-full bg-white dark:bg-gray-950 overflow-hidden"
        style={{ height: 150 }}
      >
        <div className="absolute inset-0">
          <NepseThumbnail entryDate={entry.date} onOHLC={setOhlc} />
        </div>
        ...
        <div className="absolute top-2 right-2 flex flex-col items-end gap-[3px] pointer-events-none">
          ...
          {mood && (
            <span
              className={`inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full border mt-1 ${mood.pill}`}
            >
          ...
          <div>
            ...
            <div className="flex h-1.5 rounded-full overflow-hidden gap-px bg-gray-100 dark:bg-gray-800">
              ...
              <div
                ...
                style={{ width: `${Math.round((unch / total) * 100)}%` }}
              />
              <div
        ...
        <div className="flex items-center gap-2">
          {entry.total_volume != null && (
            <div className="flex-1 bg-gray-50 dark:bg-gray-800/60 rounded-lg px-2 py-1.5">
              ...
              <p className="text-[11px] font-bold font-mono text-gray-800 dark:text-gray-200">
                {fmtVol(entry.total_volume)}
              </p>
            </div>
          )}
          {topGainer && (
            <div className="flex-1 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg px-2 py-1.5">
              ...
              <p className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 truncate">
                {topGainer.symbol}{' '}
                <span className="font-mono">+{parseFloat(topGainer.diff_pct).toFixed(1)}%</span>
              </p>
            </div>
          )}
          {topLoser && (
            <div className="flex-1 bg-red-50 dark:bg-red-900/20 rounded-lg px-2 py-1.5">
              ...
              <p className="text-[11px] font-bold text-red-500 dark:text-red-400 truncate">
                {topLoser.symbol}{' '}
                <span className="font-mono">{parseFloat(topLoser.diff_pct).toFixed(1)}%</span>
              </p>
            </div>
          )}
        </div>
            ...
            {sectors.map((s) => {
              ...
              return (
                <div
                  ...
                  style={{ background: bg }}
                >
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 truncate leading-none mb-0.5">
                    {s.name}
                  </p>
                  <p
          ...
          <div className="flex gap-1.5">
            {journalLabels.map((label, i) => (
              <div key={label} className="flex flex-col items-center gap-0.5">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${filledFields[i]?.trim() ? 'bg-amber-400' : 'bg-gray-200 dark:bg-gray-700'}`}
                />
...
function EntryCard({ entry, news, onOpen }) {
  ...
  const LABEL =
    'text-[10px] font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-1'
          ...
          <div className="flex items-center gap-3 mb-1.5 flex-wrap">
            {close != null ? (
              <div className="flex items-baseline gap-1.5">
                ...
                <span
                  className={`text-[11px] font-semibold tabular-nums ${isUp ? 'text-emerald-500' : 'text-red-400'}`}
                >
            ...
            {mood && (
              <span
                className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border ${mood.pill}`}
              >
                {mood.icon} {entry.mood}
              </span>
            )}
          ...
          {(entry.advancing != null || entry.declining != null) && (
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {entry.declining != null && (
                <span className="text-[10px] text-red-400 font-semibold">{entry.declining}↓</span>
              )}
        ...
        <div className="flex-shrink-0 flex flex-col items-end gap-2">
          <div className="flex gap-0.5">
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className={`w-1.5 h-1.5 rounded-full ${i < filledCount ? 'bg-amber-400' : 'bg-gray-200 dark:bg-gray-700'}`}
              />
            ))}
          </div>
          <svg
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 px-4 pb-4 pt-3 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              ...
              {
                label: 'Volume',
                value: entry.total_volume != null ? fmtVol(entry.total_volume) : '—',
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide ${TYPE_COLOR[item.type] || TYPE_COLOR.news}`}
                        >
                          {item.type}
                        </span>
                        {item.symbol && (
...
export default function MarketJournalTab({
  view = 'database',
  marketJournals,
  ...
  return (
    <div className="space-y-4">
      {!entries.length ? (
        ...
        <div className="space-y-2">
          {entries.map((entry) => (
            <EntryCard key={entry.id} entry={entry} news={entry.news || []} onOpen={handleOpen} />
          ))}
        </div>
      )}

      {openEntry && (
        <MarketJournalModal entry={openEntry} onClose={handleClose} onSaved={handleSaved} />
      )}
    </div>
  )
}
