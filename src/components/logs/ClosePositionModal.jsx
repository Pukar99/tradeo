...
const INPUT =
  'w-full px-3 py-2 text-[12px] rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 transition-all'
const LABEL =
...
export default function ClosePositionModal({ position, onClose, onSaved }) {
  ...
  const estPnl =
    exitNum > 0
      ? direction === 'LONG'
        ? (exitNum - wacc) * totalQty
        : (wacc - exitNum) * totalQty
      : null
  ...
  const handleBrokerFill = () => {
    ...
    setForm((f) => ({ ...f, exit_price: brokerParsed.price, date: brokerParsed.date }))
  ...
  return (
    ...
    >
      <BrokerPasteBox
        ...
        parsed={brokerParsed}
        summary={brokerParsed ? `Exit @ Rs.${brokerParsed.price} · ${brokerParsed.date}` : ''}
      ...
      >
        ...
        {estPnl !== null && (
          <div
            className={`px-4 py-2.5 rounded-xl text-[13px] font-mono font-bold text-center ${
              estPnl >= 0
