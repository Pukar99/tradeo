  ...
  return (
    ...
    >
      <div className="bg-gray-900/95 border border-gray-700 rounded-lg px-2.5 py-2 shadow-xl backdrop-blur-sm whitespace-nowrap">
        <div className="text-[10px] text-gray-400 font-mono mb-1.5 border-b border-gray-700 pb-1">
          {tooltip.date}
        </div>
        {tooltip.lines.map((line, i) => (
          <div
            ...
            style={{ marginBottom: i < tooltip.lines.length - 1 ? 2 : 0 }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full shrink-0"
              style={{ background: line.color }}
            />
            <span className="text-[10px] font-semibold" style={{ color: line.color }}>
              {line.text}
            </span>
          </div>
        ))}
...
function CandleThumbnail({ symbol, actions, wacc, ltp, isClosed, onCardHoverChange }) {
  ...
  return (
    ...
    >
      ...
      {tooltip && <TooltipBox tooltip={tooltip} containerRef={containerRef} />}
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div
            className={`absolute inset-0 z-20 transition-opacity duration-150 pointer-events-none ${hovered ? 'opacity-100' : 'opacity-0'}`}
          >
        ...
        <div className="flex flex-col gap-3 p-4 flex-1">
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <span className="font-bold text-[15px] text-gray-900 dark:text-gray-50 tracking-wide">
                  {position.symbol}
                </span>
            ...
            <div className="flex items-center gap-1 shrink-0">
              <span
                className={`flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-lg ${statusCfg.pill}`}
              >
                <span
          ...
          {hasPnl && (
            ...
            >
              ...
              <span
                className={`text-[13px] font-bold font-mono ${pnlPos ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}
              >
...
function StatBox({ label, value, mono, color }) {
  return (
    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl px-2.5 py-2">
      <div className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-0.5 truncate">
        {label}
      </div>
      <div
        className={`text-[12px] ${mono ? 'font-mono' : 'font-semibold'} truncate ${color || 'text-gray-900 dark:text-gray-200'}`}
      >
        {value}
      </div>
    </div>
...
export default function TradeGalleryView({
  positions = [],
  ltpMap = {},
  onAdd,
  onPartialExit,
