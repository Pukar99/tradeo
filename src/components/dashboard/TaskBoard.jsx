...
const FIXED_TASKS = [
  {
    id: 'fixed_1',
    title: 'Read Mindset Reminder',
    type: 'mindset',
    icon: '🧠'
  },
  {
    id: 'fixed_2',
    title: 'Check Forex Factory',
    type: 'external',
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6">
          ...
          <p className="text-blue-200 text-sm">
            Read carefully before you start trading today
          </p>
        </div>
          ...
          {phase === 'confirm' && (
            <div className="text-center">
              <p className="text-gray-600 dark:text-gray-300 text-sm mb-6">
                ...
                After you have checked it, come back here to mark this task complete.
              </p>
              <button
                onClick={handleOpen}
...
function TaskBoard({ compact = false }) {
  ...
  const [fixedTasks, setFixedTasks] = useState(
    FIXED_TASKS.map(t => ({ ...t, completed: false }))
  )
  ...
  const completedTasks =
    fixedTasks.filter(t => t.completed).length +
  ...
  const handleFixedTaskClick = async (task) => {
    ...
    if (task.type === 'mindset') {
      setActiveModal({ type: 'mindset', task })
    } else if (task.type === 'external') {
      setActiveModal({ type: 'external', task })
    } else if (task.type === 'internal') {
      await handleTaskDone(task.id)
      navigate(task.path)
    }
  }
            ...
            {customTasks.length > 0 && (
              <div className={`${space} mb-3`}>
                {customTasks.map(task => (
                  <div
                    key={task.id}
                    className="flex items-center gap-3 p-2 rounded-lg group hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    <button
