...
export default function StockSearch({ activeSymbol, onSelect, onClear }) {
  ...
  useEffect(() => {
    ...
    getMarketSymbols()
      .then(r => { setStocks(r.data?.stocks || []); setLoadErr(null) })
      .catch(() => setLoadErr('Failed to load symbols'))
  }, [open, stocks])

  // Focus the input as soon as the panel opens
  useEffect(() => {
    if (open) { setQuery(''); setCursor(0); requestAnimationFrame(() => inputRef.current?.focus()) }
  }, [open])
      ...
      <div ref={btnRef} className="flex items-center shrink-0">
        {activeSymbol ? (
          <span className="flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-md border border-blue-200 dark:border-blue-800/60 bg-blue-50 dark:bg-blue-950/30">
            <button onClick={() => setOpen(o => !o)} title="Change stock"
              className="text-[10px] font-black text-blue-600 dark:text-blue-400">
              {activeSymbol}
            </button>
            <button onClick={onClear} title="Back to index view" aria-label="Clear stock"
          ...
          <button onClick={() => setOpen(o => !o)} title="Search a stock — its monthly seasonality replaces the index heatmap"
      ...
      {open && rect && createPortal(
        <div ref={panelRef}
          ...
          <div className="shrink-0 flex items-center gap-1.5 px-2.5 py-2 border-b border-gray-100 dark:border-gray-800">
            ...
            {query && <button onClick={() => setQuery('')} className="text-gray-300 hover:text-gray-500 text-[13px] leading-none">×</button>}
          </div>
          ...
          <div className="flex-1 overflow-y-auto min-h-0 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-thumb]:bg-gray-700 [&::-webkit-scrollbar-thumb]:rounded-full">
            ...
            {stocks !== null && !loadErr && filtered.length === 0 && (
              <div className="px-3 py-4 text-center text-[11px] text-gray-400">No stocks match "{query}"</div>
            )}
            <ul ref={listRef}>
              {filtered.map((s, i) => (
                <li key={s.symbol}>
                  <button onClick={() => pick(s)} onMouseEnter={() => setCursor(i)}
                    ...
                    <div className="min-w-0">
                      <div className="text-[11px] font-black text-gray-800 dark:text-gray-100 leading-tight">{s.symbol}</div>
                      {s.company_name && (
                        <div className="text-[10px] text-gray-400 truncate leading-tight">{s.company_name}</div>
                      )}
                    </div>
                    {s.symbol === activeSymbol && (
                      <span className="shrink-0 text-[9px] font-black px-1 py-px rounded bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400">ACTIVE</span>
                    )}
                  </button>
                </li>
              ))}
          ...
          <div className="shrink-0 px-2.5 py-1 border-t border-gray-100 dark:border-gray-800">
            <span className="text-[10px] text-gray-400 dark:text-gray-500">↑↓ navigate · Enter select · Esc close</span>
          </div>
        </div>,
