...
export function SectorRow({ s, index, isActive, isBest, isWorst, history, maxAbs, onClick, pad = 'px-3 py-2' }) {
  return (
    <button
...
export function StockRow({ s, index, maxAbs, isActive, onClick, dense = false }) {
  ...
  return (
    ...
    >
      <span className={`text-[10px] text-gray-400 ${dense ? 'w-4' : 'dark:text-gray-600 w-5'} shrink-0 tabular-nums`}>{index + 1}</span>
      {dense ? (
      ...
      <div className="flex flex-1 items-center min-w-0" style={{ height: dense ? 8 : 10 }}>
        <div className="flex-1 flex justify-end">
          {!isPos && <div className={`${barH} rounded-l-sm`} style={{ width: `${barW}%`, background: color }} />}
        </div>
        <div className={`w-px ${dense ? 'h-2' : 'h-3'} bg-gray-200 dark:bg-gray-700 shrink-0 mx-0.5`} />
        <div className="flex-1 flex justify-start">
          {isPos && <div className={`${barH} rounded-r-sm`} style={{ width: `${barW}%`, background: color }} />}
        </div>
      </div>
      <span className={`text-[10px] font-bold tabular-nums ${dense ? 'w-11' : 'w-14'} text-right shrink-0`} style={{ color }}>
        {fmtPct(s.return_pct)}
      </span>
    </button>
  )
