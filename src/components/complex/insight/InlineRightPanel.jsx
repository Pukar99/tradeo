...
export default function InlineRightPanel({
  ...
  const { loading, candles, stats, sectors, available, dataError, sectorHistory } = useMonthDetail(cell, indexId, symbol)
  const [stockPop, setStockPop] = useState(null) // { symbol, rect }

  // Close the stock popover when the viewed month or index changes
  useEffect(() => { setStockPop(null) }, [cell?.year, cell?.month, indexId])
  ...
  const sectorMaxAbs = scoped?.length ? Math.max(...scoped.map(s => Math.abs(s.return_pct ?? 0)), 0.1) : 1
      ...
      <div className="shrink-0 flex items-center gap-2 px-3 py-2 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest leading-none truncate">
            {symbol && <span className="text-blue-500">{symbol} · </span>}{MONTHS_FULL[cell.month - 1]} {cell.year}
          </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain bg-gray-50 dark:bg-gray-950/70 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-thumb]:bg-gray-700 [&::-webkit-scrollbar-thumb]:rounded-full">
        ...
        {!loading && !dataError && available && (
          <div className="p-3 space-y-2.5">
              ...
              <div className="grid grid-cols-6 bg-white dark:bg-gray-900 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-800/80 shadow-sm divide-x divide-gray-100 dark:divide-gray-800">
                {[
                  ...
                  <div key={l} className="px-1.5 py-1.5 flex flex-col items-center justify-center">
                    <div className="text-[9px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-widest leading-none">{l}</div>
              ...
              <div className="rounded-xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800/80 px-3 py-2 shadow-sm">
                ...
                <div className="flex items-center justify-between mt-1">
                  <div>
                    ...
                    <div className="text-[12px] font-black tabular-nums" style={{ color: stats.nepse_return >= 0 ? '#22c55e' : '#ef4444' }}>
                  ...
                  {stats.relative_strength != null && (
                    <div className="text-right">
                      ...
                      <div className="text-[12px] font-black tabular-nums" style={{ color: stats.relative_strength >= 0 ? '#22c55e' : '#ef4444' }}>
                        {fmtPct(stats.relative_strength, 2)}
                        <span className="text-[10px] font-bold text-gray-400"> {stats.relative_strength >= 0 ? 'beat' : 'lagged'}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {scoped?.length > 0 && (() => {
              const activeS = activeSector ? scoped.find(s => s.name === activeSector.name) : null
              return (
                <div ref={sectorsCardRef} style={{ scrollMarginTop: 8 }}
                  ...
                  {!activeS ? (
                    <>
                      ...
                      <div className="flex items-center justify-between px-2.5 pt-2 pb-1.5">
                        <span className={LABEL}>{scoped.length > 1 ? `Sectors · ${scoped.length}` : 'Sector'}</span>
                        <span className="text-[10px] text-gray-400 dark:text-gray-500">tap a sector → its stocks</span>
                      </div>
                      <div>
                        {scoped.map((s, i) => (
                          <SectorRow
                            ...
                            maxAbs={sectorMaxAbs}
                            pad="px-2.5 py-1.5"
                            onClick={sec => { drillScrollRef.current = true; setActiveSector(sec) }}
                          />
                        ))}
                      </div>
                    </>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 px-2.5 py-2 border-b border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-800/30">
                        {scoped.length > 1 && (
                          <button onClick={() => setActiveSector(null)} aria-label="Back to sectors" title="Back to sectors"
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="text-[11px] font-bold text-gray-800 dark:text-gray-100 truncate leading-tight">
                            {(activeS.label || activeS.name).replace(' Sub-Index', '').replace(' Index', '')}
                          </div>
                          <div className="text-[10px] text-gray-400 leading-tight">{MONTHS_FULL[cell.month - 1]} {cell.year}</div>
                        </div>
                        <SectorMomentumSpark values={sectorHistory[activeS.name] || []} />
                        <span className="text-[12px] font-black tabular-nums shrink-0" style={{ color: sectorCol(activeS.return_pct) }}>
      ...
      {!loading && !error && stocks?.length > 0 && (
        <>
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-gray-50 dark:border-gray-800/60">
            <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400">▲ {winners}</span>
            <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-50 dark:bg-red-900/20 text-red-500">▼ {losers}</span>
            <span className="ml-auto text-[10px] text-gray-400 dark:text-gray-500">{stocks.length} stocks · tap → chart</span>
          </div>
          ...
          {filtered.length === 0 && (
            <div className="px-2.5 py-3 text-center text-[10px] text-gray-400">No stocks match "{query}"</div>
          )}
          <div>
            {filtered.map((s, i) => (
              <StockRow
