...
import {
  defaultSectorFor, useIsLg, LABEL, Skeleton,
  cellBg, cellFg, fmtPct, weightedAvg, weightedWinRate, monthStdDev,
...
export default function InsightPage() {
  ...
  useEffect(() => { safeSessionSet('tradeo_insight_index', String(selectedIndexId)) }, [selectedIndexId])
  useEffect(() => { safeSessionSet('tradeo_insight_years', yearFilter) }, [yearFilter])
  ...
  const monthMeans = useMemo(() => Array.from({ length: 12 }, (_, mi) => {
  ...
  const handleProfilePick = useCallback((year, month, value) => {
    setProfileMonth(null)
    setSelected({ year, month, value })
    setActiveSector(stock ? null : defaultSectorFor(selectedIndexId))
    setUserPicked(true)
  ...
  useEffect(() => {
    ...
    const h = e => {
      ...
      if (mobileLeftOpen) {
        if (e.key === 'Escape') { e.preventDefault(); setMobileLeftOpen(false) }
        return
      }
      // Month profile open: ← → step months (wrapping), Esc closes
      if (profileMonth) {
        if (e.key === 'ArrowLeft')  { e.preventDefault(); handleProfileNav(-1) }
        if (e.key === 'ArrowRight') { e.preventDefault(); handleProfileNav(1) }
  ...
  }, [handleNavigate, handleCloseDetail, maximized, mobileLeftOpen, profileMonth, handleProfileNav, selected])
  ...
  const toolbar = useToolbarSlot(
    <div className="flex items-center gap-2 flex-nowrap whitespace-nowrap">
      ...
      <span className={`${LABEL} normal-case hidden sm:inline ${stock ? 'max-w-[160px] truncate' : ''}`}>
        {stock
      ...
      <button onClick={() => stock ? doFetchStock(stock.symbol) : doFetch(selectedIndexId, true)} disabled={loading}
        ...
        {loading && !data ? (
          ...
          <div className="flex-1 flex items-center justify-center p-4 text-center">
            <div>
              ...
              <button onClick={() => stock ? doFetchStock(stock.symbol) : doFetch(selectedIndexId)}
              ...
              <div>
                ...
                {heroInfo && (
                  <div className="mb-2.5 flex items-center gap-x-2 gap-y-1 flex-wrap rounded-lg bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800 px-2.5 py-1.5">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-blue-500">{MONTHS_FULL[heroInfo.mi]}</span>
                    ...
                    <span className={`text-[10px] font-bold tabular-nums ${heroInfo.avg >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                      {fmtPct(heroInfo.avg)} avg
                    </span>
                    <span className="text-[10px] text-gray-200 dark:text-gray-700">·</span>
                    <span className="text-[10px] font-bold tabular-nums"
                      style={{ color: heroInfo.win >= 60 ? '#22c55e' : heroInfo.win >= 45 ? '#f59e0b' : '#ef4444' }}>
                      {heroInfo.win}% win
                    </span>
                    <span className="text-[10px] text-gray-200 dark:text-gray-700">·</span>
                    <span className="text-[10px] text-gray-500 dark:text-gray-400">#{heroInfo.rank} of {heroInfo.nMonths} months</span>
                    {/* Live market streak (consecutive ▲/▼ months) — backend computes it, was never shown */}
                    {data?.current_streak?.count > 1 && (
                      <>
                        <span className="text-[10px] text-gray-200 dark:text-gray-700">·</span>
                        <span className={`text-[10px] font-bold tabular-nums ${
                          data.current_streak.direction === 'positive' ? 'text-emerald-500' : 'text-red-500'}`}>
                          {data.current_streak.count}-mo {data.current_streak.direction === 'positive' ? '▲' : '▼'} streak
                        </span>
                      </>
                    )}
                    {heroInfo.sofar != null && (
                      <span className="ml-auto text-[10px] text-gray-500 dark:text-gray-400">
                        {curYear} so far:{' '}
                        <span className={`font-black tabular-nums ${heroInfo.sofar >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                ...
                <table className="w-full border-separate border-spacing-0">
                  ...
                  <thead>
                    <tr>
                      <th className="text-left pb-1.5 pr-2 sticky left-0 top-0 z-30 bg-white dark:bg-gray-900 translate-x-0
                        text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-10">
                        Year
                      </th>
                      {LABELS.map((m, i) => (
                        <th key={i} className="pb-1.5 px-px min-w-[36px] sticky top-0 z-20 bg-white dark:bg-gray-900">
                          <button
                  ...
                  <tbody>
                      ...
                      return (
                        <tr key={row.year}
                          ...
                          {row.months.map((val, mi) => {
                            ...
                            const isOutlier = val != null && monthMeans[mi] != null
                              && (wStdDev[mi] ?? 0) > 0 && Math.abs(val - monthMeans[mi]) > 2 * wStdDev[mi]
                            return (
                              <td key={mi} className="py-0.5 px-px">
                                <button
                                  ...
                                  style={{
                                    ...
                                    boxShadow:     isOutlier && !isSel ? `0 0 0 1.5px ${dark ? '#a78bfa' : '#8b5cf6'}` : undefined,
                                  }}>
                                  {val != null ? fmtPct(val) : '·'}
                                </button>
                              </td>
                    ...
                    {wAvg.some(v => v != null) && (
                      <tr className="border-t-2 border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                        <td className="py-1 pr-2 sticky left-0 z-10 bg-gray-100 dark:bg-gray-800 translate-x-0">
                          <span className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest whitespace-nowrap"
                            title={`Weighted average — last ${RECENT_N} years count ×2`}>Wtd Avg</span>
                        </td>
                        {wAvg.map((v, i) => (
                    ...
                    {wWinRate.some(v => v != null) && (
                      <tr>
                        <td className="py-0.5 pr-2 sticky left-0 z-10 bg-white dark:bg-gray-900 translate-x-0">
                          <span className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wide"
                            title={`Share of positive years — last ${RECENT_N} years weighted ×2`}>Win%</span>
                        </td>
                        {wWinRate.map((v, i) => {
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    <span className="flex items-center gap-px">
                      {[-10, -5, -1, 1, 5, 10, 16].map(v => (
                        <span key={v} className="w-2.5 h-2.5 rounded-[2px]" style={{ background: cellBg(v, dark) }} />
                      ))}
                    </span>
                    <span>+15%</span>
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 rounded-[2px] shrink-0"
                      style={{ boxShadow: `inset 0 0 0 1.5px ${dark ? '#a78bfa' : '#8b5cf6'}` }} />
                    &gt;2σ outlier
                  </span>
                  <span>YTD = annual return</span>
                  <span>σ = monthly volatility</span>
                  <span title={`Wtd Avg and Win% count the last ${RECENT_N} years twice`}>Wtd/Win% = recent-weighted</span>
                  <span className="ml-auto text-gray-500 dark:text-gray-500">click cell for detail · month name for profile</span>
                </div>
              </div>
            )}
