...
function ConfigRow({ row, onSaved }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
      <span className="flex-1 text-xs text-gray-600 dark:text-gray-300 font-mono truncate">{row.key}</span>
      {editing ? (
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700/50 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700/50">
        <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide">App Config</p>
      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-800/60">
        {loading ? (
          ...
          <div className="px-4 py-8 text-center text-xs text-gray-400 dark:text-gray-500">No config rows</div>
        ) : (
          display.map(row => (
            <ConfigRow key={row.key} row={row} onSaved={handleSaved} />
          ))
        )}
      </div>
    </div>
