...
// Sell side: commission + SEBON + flat DP charge.
...
export default function BacktestPartialExitModal({ session, order, candle, onClose, onExited }) {
  ...
  // A fill can only happen within the day's traded range.
  ...
  const priceOutOfRange = hasRange && xPrice > 0 && (xPrice < dayLow || xPrice > dayHigh)
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (hasRange && (xPrice < dayLow || xPrice > dayHigh))
      return setError(`Price must be within the day's range Rs.${fmt(dayLow)} – Rs.${fmt(dayHigh)}`)
  ...
  }, [session, order, candle, xQty, xPrice, reason, remQty, hasRange, dayLow, dayHigh, onExited, onClose])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-68 p-4 border border-gray-100 dark:border-gray-800">
        ...
        <div className="text-[10px] text-gray-400 mb-2">
          Holding:{' '}
          <span className="font-bold text-gray-800 dark:text-gray-200">{remQty} kittā</span>
        </div>
            ...
            <input
              ...
              className={`mt-0.5 w-full px-2 py-1.5 rounded-md border bg-white dark:bg-gray-800 dark:text-white outline-none focus:ring-1 ${
                priceOutOfRange
                  ...
                  : 'border-gray-200 dark:border-gray-700 focus:ring-yellow-400'
              }`}
            />
            {hasRange && (
              <>
                <div className="mt-1 flex items-center gap-1">
                  {[
                    ...
                    <button
                      key={p.label}
                      type="button"
                      onClick={() => setExitPrice(String(p.v))}
                      className="flex-1 px-1.5 py-0.5 text-[10px] font-semibold rounded border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-yellow-400 hover:text-yellow-600"
                    >
                      {p.label} {fmt(p.v)}
                    </button>
                  ))}
        ...
        <div className="mt-3 bg-gray-50 dark:bg-gray-800 rounded-lg p-2 text-[10px] space-y-0.5">
          <div className="flex justify-between text-gray-600 dark:text-gray-400">
            <span>Gross</span>
            <span className="dark:text-gray-300">Rs.{fmt(grossVal)}</span>
          </div>
          <div className="flex justify-between text-gray-600 dark:text-gray-400">
            <span>Fee (sell)</span>
            <span className="dark:text-gray-300">Rs.{fmt(fee)}</span>
          </div>
          <div className="flex justify-between font-bold border-t border-gray-100 dark:border-gray-700 pt-0.5 dark:text-white">
            <span>Proceeds</span>
            <span>Rs.{fmt(proceeds)}</span>
          </div>
          ...
          <div className="flex justify-between text-gray-500 dark:text-gray-400">
            <span>Remaining</span>
            <span className="dark:text-gray-300">{remaining < 0 ? '—' : remaining} kittā</span>
          </div>
        </div>

        {error && (
          <div className="mt-2 text-[10px] text-red-500 bg-red-50 dark:bg-red-900/20 rounded px-2 py-1.5">
            {error}
          </div>
        )}
        ...
        <div className="flex gap-2 mt-3">
          ...
          <button
            ...
            disabled={loading || xQty > remQty || xQty < 1 || priceOutOfRange}
