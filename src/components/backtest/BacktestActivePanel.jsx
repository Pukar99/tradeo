...
function PositionCard({ pos, currentCandle, onEditSLTP, onExit, onPartial }) {
  ...
  return (
    <div
      className={`rounded-lg border p-2 text-[10px] ${nearSL ? 'border-red-400 bg-red-50 dark:bg-red-900/10' : 'border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900'}`}
    >
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-gray-600 dark:text-gray-400 mb-1">
        <span>Qty</span>{' '}
        <span className="text-right font-medium dark:text-gray-300">{remQty} kittā</span>
        <span>Entry</span>{' '}
        <span className="text-right font-medium dark:text-gray-300">Rs.{fmt(ep)}</span>
        <span>LTP</span>{' '}
        <span className="text-right font-medium dark:text-gray-300">Rs.{fmt(ltp)}</span>
        <span>SL</span>{' '}
        <span className="text-right font-medium text-red-500">
          {pos.sl ? `Rs.${fmt(parseFloat(pos.sl))}` : <span className="text-gray-400">—</span>}
        </span>
        <span>TP</span>{' '}
        <span className="text-right font-medium text-green-600">
          {pos.tp ? `Rs.${fmt(parseFloat(pos.tp))}` : <span className="text-gray-400">—</span>}
        </span>
      </div>
      ...
      <div className="flex gap-1 mt-1.5">
        <button
          ...
          className="flex-1 py-0.5 text-[10px] font-semibold rounded border border-gray-200 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:text-gray-300"
        >
          SL/TP
        </button>
        {settled && (
          <>
            <button
              ...
              className="flex-1 py-0.5 text-[10px] font-semibold rounded border border-gray-200 dark:border-gray-700 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 dark:text-gray-300"
            >
              Partial
            </button>
            <button
              onClick={() => onExit(pos)}
              className="flex-1 py-0.5 text-[10px] font-semibold rounded border border-red-200 text-red-600 hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/20"
            >
              Exit
            </button>
          </>
...
export default function BacktestActivePanel({
  ...
  const handleAddScript = useCallback(async () => {
    ...
    setAddLoading(true)
    try {
      const res = await btAddScript(session.id, {
    ...
    <div className="flex flex-col h-full overflow-hidden text-[10px]">
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 border-b border-gray-100 dark:border-gray-800 shrink-0 flex-wrap">
        {session.scripts?.map((sc) => {
          ...
          return (
            ...
            >
              ...
              {hasPos && (
                <span
                  className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-blue-200' : 'bg-blue-500'}`}
                />
              )}
            </button>
          )
        ...
        <button
          ...
          className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700"
        >
          + Add
        </button>
      </div>
      ...
      {addingScript && (
        <div className="px-2 py-2 border-b border-gray-100 dark:border-gray-800 bg-blue-50 dark:bg-blue-900/10 shrink-0">
          ...
          <input
            ...
            className="w-full px-2 py-1 text-[10px] rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white outline-none mb-1"
          />
          {addErr && <div className="text-red-500 text-[10px] mb-1">{addErr}</div>}
          <div className="flex gap-1">
            <button
              onClick={handleAddScript}
              disabled={addLoading}
              className="flex-1 py-1 text-[10px] font-bold bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50"
            >
              {addLoading ? 'Adding…' : 'Add Script'}
            </button>
            <button
              onClick={() => setAddingScript(false)}
              className="flex-1 py-1 text-[10px] font-bold border border-gray-200 dark:border-gray-700 rounded text-gray-500"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-2 min-h-0">
        ...
        <div>
          ...
          {openPositions.length === 0 ? (
            <div className="text-[10px] text-gray-400 py-2 text-center italic">
              No open positions
            </div>
          ) : (
        ...
        {/* Session meta */}
        <div className="text-[10px] text-gray-400 text-center pt-1 border-t border-gray-100 dark:border-gray-800">
          SL Mode:{' '}
          <span
            className={`font-bold ${session.sl_mode === 'AUTO' ? 'text-orange-500' : 'text-blue-500'}`}
          >
            {session.sl_mode}
          </span>
          {' · '}
        ...
        {/* End session */}
        {endErr && <p className="text-[10px] text-red-500 text-center mb-1">{endErr}</p>}
        {!confirmEnd ? (
          <button
          ...
          <div className="rounded-md border border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-900/10 p-2">
            <div className="text-[10px] text-red-600 dark:text-red-400 mb-1.5 text-center">
              Open positions will be marked ABANDONED. Are you sure?
            </div>
            <div className="flex gap-1">
              <button
                onClick={handleEndSession}
                className="flex-1 py-1 text-[10px] font-bold bg-red-600 text-white rounded hover:bg-red-700"
              >
                End & Report
              </button>
              <button
