  ...
  return (
    <ul className="space-y-1.5 mt-2">
      {lines.map((line, i) => (
        <li
          key={i}
          className="flex items-start gap-2 text-[11px] text-gray-600 dark:text-gray-300 leading-relaxed"
        >
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-gray-400">
              {loading
                ...
                : profile
                  ? `${s?.totalTrades} trades · ${s?.winRate}% WR · updated`
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 px-4 py-4 space-y-4">
          ...
          {error && (
            <div className="text-[11px] text-red-500 text-center py-2">
              {error} —{' '}
              <button onClick={loadProfile} className="underline hover:text-red-600">
                retry
              </button>
            </div>
          )}

          {reason && <p className="text-[11px] text-gray-400 text-center py-4">{reason}</p>}
            ...
            <>
              <div className="grid grid-cols-4 gap-2">
                <StatPill
                  label="Win Rate"
                  value={`${s.winRate}%`}
                  color={parseFloat(s.winRate) >= 50 ? 'text-emerald-500' : 'text-red-500'}
                />
                <StatPill
                  label="Avg Win"
                  value={`Rs.${parseFloat(s.avgWinPnl).toLocaleString()}`}
                  color="text-emerald-500"
                />
                <StatPill
                  label="Avg Loss"
                  value={`Rs.${Math.abs(parseFloat(s.avgLossPnl)).toLocaleString()}`}
                  color="text-red-500"
                />
                      ...
                      {profile.bestSymbols.map((s) => (
                        ...
                        >
                          ...
                          <span className="text-emerald-500 font-semibold">
                            +Rs.{parseFloat(s.pnl).toLocaleString()}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(profile.tagStats).map(([tag, s]) => {
                      ...
                      return (
                        <div
                          key={tag}
                          className="flex items-center gap-1.5 text-[10px] bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg px-2.5 py-1.5"
                        >
                          <span className="font-semibold text-gray-700 dark:text-gray-200">
                            {tag}
                          </span>
                          <span
                            className={`font-bold ${wr >= 50 ? 'text-emerald-500' : 'text-red-500'}`}
                          >
                            {wr}%
                          </span>
              ...
              <div className="flex items-center justify-between">
                <p className="text-[10px] text-gray-300 dark:text-gray-600">
                  Generated{' '}
                  {profile.generatedAt ? new Date(profile.generatedAt).toLocaleTimeString() : ''} ·
                  cached 1hr
                </p>
                <button
                  onClick={loadProfile}
