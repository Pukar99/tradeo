...
import { closePosition, removeFromWatchlist, updateWatchlist } from '../../api'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-xs z-10 p-5 text-center animate-modal-in">
        ...
        {err && (
          <p className="text-[11px] text-red-500 mb-3 bg-red-50 dark:bg-red-950/40 px-3 py-2 rounded-xl">
            {err}
          </p>
        )}
        <div className="flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-[11px] font-semibold text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
            Cancel
          </button>
          <button
...
function EditWatchItemForm({ item, onClose, onSaved }) {
  ...
  const inp =
    'w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-[12px] text-gray-800 dark:text-gray-200 outline-none focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-800 transition-all'
    ...
    <form onSubmit={handleSave} className="px-4 py-4 space-y-3">
      ...
      <div className="flex gap-2 pt-1">
        <button
          type="button"
          onClick={onClose}
          className="flex-1 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-[11px] font-medium text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        >
          Cancel
        </button>
        <button
          type="submit"
          disabled={saving}
          className="flex-1 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-semibold disabled:opacity-50 transition-colors"
        >
          {saving ? 'Saving…' : 'Save'}
        </button>
      </div>
...
const PositionCard = memo(function PositionCard({ p, selected, onSelect, latestPrice, index = 0 }) {
  ...
  const progressPct =
    sl && tp && lp ? Math.min(100, Math.max(0, ((lp - sl) / (tp - sl)) * 100)) : null
  ...
  return (
    ...
    >
      <div className="flex overflow-hidden rounded-xl">
        ...
        <div
          className={`w-[3px] shrink-0 rounded-l-xl ${isLong ? 'bg-emerald-400' : 'bg-red-400'}`}
        />
        <div className="flex-1 px-2.5 py-2">
          ...
          <div className="flex items-center justify-between mb-1.5" translate="no">
            ...
            {pnlPct !== null ? (
              <span
                className={`text-[11px] font-black tabular-nums ${isPos ? 'text-emerald-400' : 'text-red-400'}`}
              >
                {isPos ? '+' : ''}
                {pnlPct.toFixed(2)}%
              </span>
          ...
          <div className="grid grid-cols-3 gap-1 mb-1.5" translate="no">
            <div>
              ...
              <p className="text-[10px] font-semibold text-blue-400 tabular-nums">
                {entry.toLocaleString()}
              </p>
            </div>
            <div>
              <p className="text-[8px] text-gray-400 uppercase tracking-wide">Qty</p>
              <p className="text-[10px] font-semibold text-gray-700 dark:text-gray-300 tabular-nums">
            ...
            {sl && (
              <div>
                ...
                <p className="text-[10px] font-semibold text-red-400 tabular-nums">
                  {sl.toLocaleString()}
                </p>
              </div>
            )}
            {tp && (
              <div>
                ...
                <p className="text-[10px] font-semibold text-emerald-500 tabular-nums">
                  {tp.toLocaleString()}
                </p>
              </div>
            )}
  ...
  return (
    <>
      ...
      >
        <div className="flex overflow-hidden rounded-xl">
          <div
            className={`w-0.5 shrink-0 rounded-l-xl ${isPre ? 'bg-amber-400' : 'bg-blue-400'}`}
          />
          <div className="flex-1 px-2.5 py-2">
            <div className="flex items-center justify-between" translate="no">
              <span className="text-[11px] font-bold text-gray-800 dark:text-gray-100 group-hover:text-blue-500 transition-colors">
                {w.symbol}
              </span>
              <span
                className={`text-[8px] font-bold px-1.5 py-0.5 rounded-md ${
              ...
              <div className="flex gap-3 mt-1 text-[9px]" translate="no">
                {w.watch_low && (
                  <span className="text-gray-400">
                    Low <span className="text-red-400 font-semibold">{w.watch_low}</span>
                  </span>
                )}
                {w.watch_high && (
                  <span className="text-gray-400">
...
export default function LeftPanel() {
  ...
  const loadWatchlist = useCallback(() => {
    ...
    getWatchlist()
      .then((r) => {
        ...
        setWatchlist(
          (r.data || []).filter(
            (w) => w.category === 'active' || w.category === 'pre' || w.category === 'pre-watch'
          )
        )
      })
      .catch(() => {})
      .finally(() => {
        if (mountedRef.current) setWatchLoading(false)
      })
  }, [])
  ...
  const loadData = useCallback(() => {
    refreshPositions()
    loadWatchlist()
  }, [refreshPositions, loadWatchlist])

  // Skip for guests: /api/dashboard/watchlist is auth-gated, so a guest fetch 401s
  // and the global interceptor would force a "session expired" redirect — hiding
  // the General tab a guest is allowed to browse. No user → no watchlist to load.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    getBatchPrices(symbols)
      .then((res) => {
        ...
        const prices = {}
        symbols.forEach((s) => {
          if (priceMap[s]?.price) prices[s] = parseFloat(priceMap[s].price)
        })
        ...
        const alerts = withAlerts.reduce((acc, p) => {
          ...
          if (p.sl) {
            ...
            if (Math.abs((price - slV) / slV) <= ALERT_PCT_THRESHOLD)
              found.push({ type: 'SL', label: 'Near SL', threshold: slV })
          }
          if (p.tp) {
            const tpV = parseFloat(p.tp)
            if (Math.abs((price - tpV) / tpV) <= ALERT_PCT_THRESHOLD)
              found.push({ type: 'TP', label: 'Near TP', threshold: tpV })
          }
  ...
  useEffect(() => {
    ...
    const fn = (e) => {
      if (e.key === 'Escape') setEditWatchItem(null)
    }
            ...
            <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-sm z-10 overflow-hidden animate-modal-in">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
                <div>
                  ...
                  <p className="text-[10px] text-gray-400" translate="no">
                    {editWatchItem.symbol}
                  </p>
                </div>
                <button
                  onClick={() => setEditWatchItem(null)}
                  className="w-6 h-6 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 text-[16px]"
                >
                  ×
                </button>
              </div>
              <EditWatchItemForm
                item={editWatchItem}
                onClose={() => setEditWatchItem(null)}
                onSaved={() => {
                  setEditWatchItem(null)
                  loadWatchlist()
                }}
              />
            </div>
            ...
            {positionsLoading ? (
              <div className="space-y-1.5 pt-1">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-gray-100 dark:border-white/8 bg-white/40 dark:bg-white/5 px-3 py-3 animate-pulse"
                  >
            ...
            {watchLoading ? (
              <div className="space-y-1.5 pt-1">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-gray-100 dark:border-white/8 bg-white/40 dark:bg-white/5 px-3 py-2.5 animate-pulse"
                  >
              ...
              watchlist.map((w) => {
                if (pendingDelete === w.id)
                  return (
                    <div
                      key={w.id}
                      className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/15 px-3 py-2.5"
                    >
        ...
        {alertPositions.length > 0 && (
          <div className="space-y-1 max-h-[120px] overflow-y-auto">
            {alertPositions.map((p) => (
              <div
                key={p.id}
                className="rounded-xl border border-orange-200 dark:border-orange-700/70 bg-orange-50 dark:bg-orange-950/25 px-2.5 py-2"
              >
                <div className="flex items-center justify-between mb-0.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold text-gray-800 dark:text-gray-100 hover:text-orange-500 transition-colors"
                      translate="no"
                    >
                      {p.symbol}
                    </button>
                ...
                {p._alerts.map((a, j) => (
                  <p
                    key={j}
                    className={`text-[9px] font-semibold ${a.type === 'SL' ? 'text-red-500' : 'text-emerald-500'}`}
                  >
