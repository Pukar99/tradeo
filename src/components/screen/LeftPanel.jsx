      ...
      <div className="h-1/2 flex flex-col min-h-0 border-b border-gray-100 dark:border-gray-800">
        ...
        <div className="flex bg-gray-100 dark:bg-gray-800 rounded-xl p-0.5 m-2 shrink-0">
          {[['portfolio', 'P'], ['watchlist', 'W']].map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)}
              className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-colors relative ${
        ...
        {tab === 'portfolio' && (
          <div className="flex-1 overflow-y-auto min-h-0 px-2 space-y-1">
            {positions.length === 0 ? (
              <p className="text-center text-[10px] text-gray-400 py-4">No open positions</p>
            ) : positions.map(p => (
        ...
        {tab === 'watchlist' && (
          <div className="flex-1 overflow-y-auto min-h-0 px-2 space-y-1">
            {watchlist.length === 0 ? (
              <p className="text-center text-[10px] text-gray-400 py-4">No watchlist stocks</p>
            ) : watchlist.map(w => {
              ...
              if (pendingDelete === w.id) return (
                <div key={w.id} className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/15 px-2 py-2">
                  <p className="text-[10px] font-semibold text-red-600 dark:text-red-400 mb-1.5 text-center">
                    Remove <span translate="no">{w.symbol}</span> from watchlist?
                  </p>
                  <div className="flex gap-1">
                    <button
                      className="flex-1 py-1 text-[10px] font-bold bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"
                      onClick={async () => {
                        try {
                    ...
                    <button
                      className="flex-1 py-1 text-[10px] font-semibold border border-gray-200 dark:border-gray-700 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
      ...
      <div className="h-1/2 flex flex-col gap-2 px-2 py-3 min-h-0">
        ...
        {watchErr && (
          <div className="flex items-center justify-between bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg px-2 py-1 shrink-0">
            <p className="text-[10px] text-red-500">{watchErr}</p>
        ...
        {alertPositions.length > 0 ? (
          <div className="flex-1 overflow-y-auto min-h-0 space-y-1.5">
            {alertPositions.map((p) => (
              <div key={p.id} className="rounded-xl border border-orange-200 dark:border-orange-900 bg-orange-50 dark:bg-orange-950/30 px-2 py-1.5">
                ...
                {p._alerts.map((a, j) => (
                  <p key={j} className={`text-[10px] font-medium ${a.type === 'SL' ? 'text-red-500' : 'text-emerald-500'}`}>
          ...
          <p className="text-center text-[10px] text-gray-300 dark:text-gray-700 mt-auto">No SL/TP alerts</p>
        )}
      </div>
