...
function renderDrawings(ctx, canvas, chart, priceSeries, drawings, preview, isDark) {
  ...
  for (const d of drawings) {
    if (d.type === 'horizontal') {
      ...
      ctx.font = 'bold 9px monospace'
      ctx.fillText(Number(d.price).toFixed(2), canvas.width - 54, p.y - 3)
      ctx.restore()
      ...
      ctx.fillStyle = d.color
      ctx.font = '9px monospace'
      ctx.fillText(d.time, p.x + 4, 14)
      ctx.restore()
    } else if (d.type === 'trendline') {
      const p1 = c2p(d.t1, d.p1),
        p2 = c2p(d.t2, d.p2)
      drawLine(p1, p2, d.color)
      dot(p1, d.color)
      dot(p2, d.color)
    } else if (d.type === 'ray') {
      const p1 = c2p(d.t1, d.p1),
        p2 = c2p(d.t2, d.p2)
      if (p1 && p2) {
        drawLine(p1, extendRay(p1, p2), d.color)
        dot(p1, d.color)
      }
    } else if (d.type === 'fib') {
...
function PositionBadge({ positions, latestClose }) {
  ...
  const avgEntry =
    totalQty > 0
      ? positions.reduce(
          (s, p) => s + parseFloat(p.entry_price) * (p.remaining_quantity ?? p.quantity ?? 0),
          0
        ) / totalQty
      : 0
    ...
    <div className="absolute bottom-16 left-3 z-20 pointer-events-none" translate="no">
      <div className="bg-white/96 dark:bg-gray-900/96 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-2xl shadow-lg overflow-hidden w-56">
        ...
        >
          ...
          {close > 0 && (
            <span
              className={`text-[11px] font-bold tabular-nums ${isPos ? 'text-emerald-500' : 'text-red-400'}`}
            >
              {isPos ? '+' : ''}
              {pnlPct.toFixed(2)}%
            </span>
          ...
          {positions.map((pos, idx) => {
            ...
            return (
              <div key={pos.id ?? idx} className="flex items-start gap-1.5">
                <div
                  className={`w-1.5 h-1.5 rounded-full mt-0.5 shrink-0 ${ENTRY_DOT_COLORS[idx % ENTRY_DOT_COLORS.length]}`}
                />
                <div className="grid grid-cols-3 gap-x-2 gap-y-0.5 flex-1">
                  <div>
                    ...
                    <p
                      className={`text-[10px] font-semibold tabular-nums ${ENTRY_TEXT_COLORS[idx % ENTRY_TEXT_COLORS.length]}`}
                    >
                      {e.toFixed(2)}
                    </p>
                  </div>
                  ...
                  {pos.sl && (
                    <div>
                      <p className="text-[9px] text-gray-400 uppercase tracking-widest">SL</p>
                  ...
                  {pos.tp && (
                    <div>
                      <p className="text-[9px] text-gray-400 uppercase tracking-widest">TP</p>
                  ...
                  {rr && (
                    <div>
                      <p className="text-[9px] text-gray-400 uppercase tracking-widest">R:R</p>
            ...
            return (
              <div className="px-3 pb-2.5">
                <div className="relative h-1.5 rounded-full overflow-visible mb-1">
                  ...
                  />
                  <div
                    className={`absolute w-2.5 h-2.5 rounded-full border-2 border-white dark:border-gray-900 shadow ${isPos ? 'bg-emerald-400' : 'bg-red-400'}`}
                ...
                <div className="flex justify-between text-[9px]">
                  ...
                  <span
                    className={`font-semibold tabular-nums ${isPos ? 'text-emerald-400' : 'text-red-400'}`}
                  >
                    {close.toFixed(2)}
                  </span>
    ...
    <div className="w-full h-full flex flex-col gap-2 p-4 animate-pulse bg-white dark:bg-gray-900">
      <div className="flex gap-1 items-end h-full">
        {Array.from({ length: 40 }).map((_, i) => (
          <div
            key={i}
            className="flex-1 bg-gray-200 dark:bg-gray-800/80 rounded-sm"
...
export default function StockChart({
    ...
    (drawings) => {
      try {
        ...
        if (err?.name === 'QuotaExceededError') {
          console.warn(
            '[Tradeo] Drawing storage full — drawings not saved. Clear browser storage to resume.'
          )
        }
  ...
  useEffect(() => {
    ...
    smcSeriesRef.current.forEach((s) => {
      try {
    ...
    function addLine(fromDate, toDate, level, color, lineStyle = 1, lineWidth = 1.5) {
      try {
        ...
        s.setData([
          { time: fromDate, value: level },
          { time: toDate, value: level },
        ])
    ...
    if (smcToggles.showBOS) {
      smcData.bos
        ...
        .slice(-5)
        .forEach((b) => addLine(b.swing_date ?? b.date, b.date, b.level, '#22c55e', 0, 1.5))
    }
    ...
    if (smcToggles.showCHoCH) {
      smcData.choch
        ...
        .slice(-5)
        .forEach((c) => addLine(c.swing_date ?? c.date, c.date, c.level, '#f59e0b', 0, 2))
    }
    ...
    if (smcToggles.showOB) {
      smcData.order_blocks
        ...
        .slice(-5)
        .forEach((o) => addZoneBand(o.date, o.high, o.low, '#22c55e'))
    }
    ...
    if (smcToggles.showFVG) {
      smcData.fvg
        ...
        .slice(-5)
        .forEach((f) => addZoneBand(f.date, f.top, f.bottom, '#3b82f6'))
    }
    // Sweeps: line from swing origin to sweep candle only
    if (smcToggles.showSweeps) {
      smcData.sweeps
        .filter((s) => s.type === 'buy_side')
        .slice(-5)
        .forEach((s) => addLine(s.swing_date ?? s.date, s.date, s.level, '#a78bfa', 0, 1.5))
    }
    if (smcToggles.showEntry && smcSignals?.length && priceSeriesRef.current) {
      try {
        ...
        priceSeriesRef.current.setMarkers(
          [...existing.filter((m) => m.shape !== 'arrowUp'), ...newMarkers].sort((a, b) =>
  ...
  useEffect(() => {
    ...
    const addZoneBand = (fromDate, top, bottom, color) => {
      ...
      try {
        ...
        sTop.setData([
          { time: fromDate, value: top },
          { time: lastDate, value: top },
        ])
        ...
        sBot.setData([
          { time: fromDate, value: bottom },
          { time: lastDate, value: bottom },
        ])
    ...
    if (paToggles.showSwings && paData.swings?.length && priceSeriesRef.current) {
      try {
        ...
        const merged = [
          ...existing.filter((m) => !['HH', 'HL', 'LH', 'LL'].includes(m.text)),
          ...swingMarkers,
    ...
    if (paToggles.showVolume && paData.volume_spikes?.length && priceSeriesRef.current) {
      try {
        ...
        const merged = [...existing.filter((m) => !m.text?.endsWith('×')), ...volMarkers].sort(
    ...
    if (paToggles.showPatterns && paData.patterns?.length && priceSeriesRef.current) {
      ...
      try {
        ...
        const merged = [...existing.filter((m) => !ptTypes.has(m.text)), ...ptMarkers].sort(
          (a, b) => (a.time < b.time ? -1 : a.time > b.time ? 1 : 0)
        )
        priceSeriesRef.current.setMarkers(merged)
      } catch (_) {}
    ...
    req
      .then(async (r) => {
        ...
        if (data.length > 0) {
          ...
          const now = nptNow()
          const afterClose =
            now.getUTCHours() > 15 || (now.getUTCHours() === 15 && now.getUTCMinutes() >= 10)
  ...
  useEffect(() => {
    ...
    loadLC().then(({ createChart, CrosshairMode, LineStyle }) => {
      ...
      if (activeIndicators.includes('BB')) {
        ...
        if (upper.length) {
          const sU = main.addLineSeries({
            color: '#a78bfa88',
            lineWidth: 1,
            priceLineVisible: false,
            title: 'BB+',
          })
          const sL = main.addLineSeries({
            color: '#a78bfa88',
            lineWidth: 1,
            priceLineVisible: false,
            title: 'BB-',
          })
          const sM = main.addLineSeries({
            color: '#a78bfa55',
            lineWidth: 1,
            lineStyle: 2,
            priceLineVisible: false,
      ...
      if (activeIndicators.includes('RSI') && rsiRef.current) {
        ...
        if (rsiData.length) {
          ...
          rc.addLineSeries({ color: C.rsi, lineWidth: 1.5, priceLineVisible: false }).setData(
            rsiData
          )
          rc.addLineSeries({
            color: C.down + '80',
      ...
      if (activeIndicators.includes('MACD') && macdRef.current) {
        ...
        if (macd.length) {
          ...
          mc.addLineSeries({ color: C.signal, lineWidth: 1.5, priceLineVisible: false }).setData(
            signal
          )
      ...
      const applyZoom = () => {
        ...
        if (activePositions?.length) {
          ...
          activePositions.forEach((p) => {
            ...
            rows.forEach((row) => {
              if (row.date) allEntryDates.push(row.date.slice(0, 10))
            })
          })
  ...
  useEffect(() => {
    ...
    return () => {
      if (rafRef.current) cancelAnimationFrame(rafRef.current)
      try {
        chart.timeScale().unsubscribeVisibleTimeRangeChange(scheduleRepaint)
      } catch (_) {}
      ro.disconnect()
    }
      ...
      if (tool === 'trendline' || tool === 'ray' || tool === 'fib') {
        ...
        if (moved) {
          drawingsRef.current.push({
            type: tool,
            color: preview.color,
            t1: preview.start.time,
            p1: preview.start.price,
            t2: pt.time,
            p2: pt.price,
          })
          saveDrawingsRef.current(drawingsRef.current)
  ...
  const mainPct =
    indCount === 0 ? 100 : indCount === 1 ? 70 : indCount === 2 ? 55 : indCount === 3 ? 45 : 40
  ...
  if (error)
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-4">
        ...
        <button onClick={handleRefresh} className="text-[10px] text-blue-500 hover:underline">
          Retry
        </button>
      </div>
    )
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 pointer-events-none">
                <span className="text-[10px] text-white bg-amber-600/90 px-2.5 py-1 rounded-full shadow whitespace-nowrap">
                  {activeTool === 'horizontal'
                    ? 'Click to place · Esc to exit · Right-click drawing to delete'
                    : activeTool === 'vertical'
                      ? 'Click to place · Esc to exit · Right-click drawing to delete'
                      : activeTool === 'path'
                        ? 'Click points · Double-click to finish · Right-click to cancel · Esc to exit'
                        : activeTool === 'fib'
                          ? 'Click & drag to set Fibonacci range · Right-click to cancel · Esc to exit'
            ...
            {showRSI && (
              ...
              >
                <SubPaneLabel
                  ...
                  color="#a78bfa"
                  legend={[
                    { color: '#ef444480', label: '70 OB' },
                    { color: '#10b98180', label: '30 OS' },
                  ]}
                />
                <div ref={rsiRef} className="w-full flex-1 min-h-0 relative">
                  {!chartsRef.current.rsi && (
                    <div className="absolute inset-0 animate-pulse bg-white dark:bg-gray-900" />
                  )}
                </div>
              </div>
            )}
