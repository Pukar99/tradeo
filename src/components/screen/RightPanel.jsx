          ...
          <div className={`rounded-xl px-2.5 py-2 border ${
            ...
            <div className="flex items-baseline gap-2 mb-1.5">
              <span className="text-[15px] font-black text-gray-900 dark:text-white tabular-nums">
                {(nepse.close ?? 0).toLocaleString('en-NP', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
              <span className={`text-[10px] font-semibold tabular-nums ${isPos ? 'text-emerald-500' : 'text-red-400'}`}>
                {ptChg >= 0 ? '+' : ''}{(ptChg ?? 0).toFixed(2)} pts
              </span>
            </div>
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="px-2 pt-2 pb-1.5 shrink-0">
        {isPinned ? (
          <div className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/60 rounded-xl px-2.5 py-1.5">
            ...
            <span className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 flex-1 truncate tabular-nums">{clickedMovers.date}</span>
            <button onClick={clearPin} className="text-[9px] font-bold text-blue-400 hover:text-red-400 transition-colors">✕ Unpin</button>
          </div>
        ) : datesErr ? (
          ...
          <div className="flex items-center bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700/60 overflow-hidden">
            ...
            <div className="flex-1 text-center">
              <span className="text-[10px] font-bold text-gray-700 dark:text-gray-200 tabular-nums">{selectedDate || '—'}</span>
            </div>
      ...
      <div className="flex gap-0.5 px-2 mb-2 shrink-0 bg-white/25 dark:bg-white/[0.06] backdrop-blur-sm rounded-xl p-0.5 border border-white/20 dark:border-white/[0.06]">
        {Object.entries(TAB_CONFIG).map(([key, cfg]) => (
          ...
          >
            {moverTab === key && <span className={`inline-block w-1 h-1 rounded-full ${cfg.dot} mr-1 mb-px`} />}
            {cfg.label}
          </button>
        ))}
      </div>
            ...
            {loading && !isPinned ? (
              ...
              <div className="space-y-0 animate-fade-up" translate="no">
                {moverTab === 'gainers' && gainers.slice(0, 10).map((s, i) => (
                  <div key={s.s} onClick={() => selectSymbol(s.s)}
                    ...
                    <span className="text-[9px] text-gray-300 dark:text-gray-600 tabular-nums w-3 shrink-0">{i + 1}</span>
                    <span className="flex-1 text-[10px] font-bold text-gray-800 dark:text-gray-100 group-hover:text-blue-500 transition-colors truncate">{s.s}</span>
                    <span className="text-[9px] text-gray-400 tabular-nums shrink-0">{s.c?.toLocaleString()}</span>
                    <ChangeBar value={s.p} />
                  </div>
                ))}
                ...
                {moverTab === 'volume' && volData.slice(0, 10).map((s, i) => (
                  <div key={s.s} onClick={() => selectSymbol(s.s)}
                    ...
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="text-[9px] text-gray-300 dark:text-gray-600 tabular-nums w-3 shrink-0">{i + 1}</span>
                      <span className="flex-1 text-[10px] font-bold text-gray-800 dark:text-gray-100 group-hover:text-blue-500 transition-colors truncate">{s.s}</span>
                      <span className="text-[9px] text-gray-400 tabular-nums shrink-0 whitespace-nowrap">{isNaN(parseFloat(s.t)) ? '—' : (parseFloat(s.t) / 1e6).toFixed(1) + 'M'}</span>
                      <ChangeBar value={s.p} />
                    </div>
            ...
            {!loading && (activeMovers || volData.length > 0) && (
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-semibold text-emerald-500">▲ {gainers.length}</span>
                  <span className="text-[9px] font-semibold text-red-400">▼ {losers.length}</span>
                  {activeMovers?.total && <span className="text-[9px] text-gray-400">/ {activeMovers.total}</span>}
                </div>
                {moverTab !== 'volume' && (
                  <button onClick={() => setShowAllMovers(true)}
      ...
      <div className="px-2 shrink-0">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-bold text-gray-600 dark:text-gray-300">Market Intel</span>
          </div>
          {feedItems.length > 0 && (
            <span className="text-[9px] bg-violet-100 dark:bg-violet-900/40 text-violet-500 px-1.5 py-0.5 rounded-full font-semibold">{feedItems.length}</span>
          )}
        </div>
