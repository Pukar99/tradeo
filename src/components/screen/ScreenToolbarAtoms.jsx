...
export function ToolbarSymbolSearch({ symbols, stocksOnly = false, onSelect }) {
  ...
  const allItems = useMemo(() => {
    const indexes = stocksOnly ? [] : (symbols?.indexes ?? []).map(i => ({
      label: i.name, sub: 'Index', indexId: i.index_id, companyName: null,
    }))
    const stocks = (symbols?.stocks ?? []).map(s => ({
      label: s.symbol, sub: 'Stock', indexId: null, companyName: s.company_name || null,
    }))
    return [...indexes, ...stocks]
  }, [symbols, stocksOnly])

  const q = query.toLowerCase()
  const filtered = useMemo(() => query.length < 1
    ? allItems.slice(0, 20)
    : allItems
        .filter(i =>
        ...
        .slice(0, 30)
  , [allItems, q, query.length]) // eslint-disable-line react-hooks/exhaustive-deps
...
export function ToolbarTimeframes({ frames = DEFAULT_TIMEFRAMES, onChange }) {
  ...
  const handleClick = useCallback((tf) => {
    setTimeframe?.(tf)
    onChange?.(tf)
  ...
  return (
    <div className="flex items-center gap-0.5 bg-gray-100 dark:bg-gray-800 rounded-md p-0.5 shrink-0">
      {frames.map(tf => (
        <button key={tf} onClick={() => handleClick(tf)}
...
export function ToolbarConfigButton({ label = 'Config', children }) {
  ...
  return (
    <div ref={triggerRef}>
      <button
        onClick={() => { setOpen(v => !v); updateRect() }}
        className={`px-2 py-0.5 rounded text-[10px] font-semibold transition-all border whitespace-nowrap ${
          open
